...
test_mysql:
	...
	TEST_DRIVER="mysql" TEST_DATASOURCE="root@tcp(127.0.0.1:3306)/test" go test -short github.com/armab/drone/server/datastore/database
	mysql -P 3306 --protocol=tcp -u root -e 'drop database test;'

test_postgres:
	TEST_DRIVER="postgres" TEST_DATASOURCE="host=127.0.0.1 user=postgres dbname=postgres sslmode=disable" go test -short github.com/armab/drone/server/datastore/database

build:
	mkdir -p packaging/output
	mkdir -p packaging/root/usr/local/bin
	go build -o packaging/root/usr/local/bin/drone  -ldflags "-X main.revision $(SHA) -X main.version $(VERSION)" github.com/armab/drone/cli
	go build -o packaging/root/usr/local/bin/droned -ldflags "-X main.revision $(SHA) -X main.version $(VERSION)" github.com/armab/drone/server
...
embed:
	rice --import-path="github.com/armab/drone/server" append --exec="packaging/root/usr/local/bin/droned"
...
deb:
	fpm -s dir -t deb -n drone -v $(VERSION) -p packaging/output/drone.deb \
		...
		--url https://github.com/armab/drone \
